...
from rtdetr_detection_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
...
_CONFIG = json.dumps({"model_type": "rt_detr", "num_queries": 300, "use_pretrained_backbone": False}).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        RTDetrDetectionPipeline.from_pretrained(device="cpu", weights_dir=tmp_path, allow_download=False)
