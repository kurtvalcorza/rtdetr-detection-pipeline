...
def _fake_pipeline(calls: list | None = None) -> RTDetrDetectionPipeline:
    ...
    return RTDetrDetectionPipeline(runner, "cpu")
...
def test_detect_output_fields_and_defaults():
    ...
    assert [d["label"] for d in result["detections"]] == ["stop sign", "clock"]  # sorted by score desc
...
def test_detect_rejects_malformed_backend_output():
    bad_box = RTDetrDetectionPipeline(
        lambda *_: [{"box": [0, 0, 1], "label": "stop sign", "score": 0.1}], "cpu"
    )
    with pytest.raises(RuntimeError):
        bad_box.detect(Image.new("RGB", (64, 64)))
    bad_label = RTDetrDetectionPipeline(
        lambda *_: [{"box": [0, 0, 1, 1], "label": "unicorn", "score": 0.1}], "cpu"
    )
    with pytest.raises(RuntimeError, match="malformed"):
        bad_label.detect(Image.new("RGB", (64, 64)))
    too_many = RTDetrDetectionPipeline(
        lambda *_: [{"box": [0, 0, 1, 1], "label": "stop sign", "score": 0.5}] * (MAX_DETECTIONS + 1), "cpu"
    )
